...
function App() {
  ...
  if (isCheckoutRoute) {
    ...
    // Helper para escolher o link da Hotmart conforme o plano
    const getHotmartLink = (plan: string) => {
      const p = plan.toLowerCase();
      if (p.includes('básico') || p.includes('basico') || p.includes('basic')) {
        return 'https://pay.hotmart.com/A102075268K?off=90oayl4p';
      }
      if (p.includes('premium')) {
        return 'https://pay.hotmart.com/O102075592R?off=kirw85pm';
      }
      return 'https://pay.hotmart.com/C102075545K?off=72qtduc2'; // padrão: Pro
    };
        ...
        <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-16 mt-20 lg:mt-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 max-w-7xl mx-auto">
            ...
            <div className="space-y-6 lg:space-y-8">
                ...
                <div className="space-y-3 lg:space-y-4">
                  <div className="flex justify-between items-center p-3 lg:p-4 bg-white/5 rounded-xl lg:rounded-2xl">
                    ...
                    <span className="text-white font-bold text-base lg:text-lg">
                      <span className="text-white/60 line-through mr-2">R$ {(planData.name === 'Plano Básico' ? 35 : planData.name === 'Plano Pro' ? 45 : 75).toFixed(2)}</span>
                      R$ 10,00
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-3 lg:p-4 bg-green-500/10 rounded-xl lg:rounded-2xl">
                    <span className="text-green-400 font-medium text-sm lg:text-base">
                      Desconto Promoção (tempo limitado)
                    </span>
                    <span className="text-green-400 font-bold text-base lg:text-lg">-R$ {((planData.name === 'Plano Básico' ? 35 : planData.name === 'Plano Pro' ? 45 : 75) - 10).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center p-4 lg:p-6 bg-gradient-to-r from-[#FF3002]/10 to-[#E02702]/10 rounded-xl lg:rounded-2xl border border-[#FF3002]/20">
                    ...
                    <span className="text-[#FF3002] font-bold text-xl lg:text-2xl">R$ 10,00</span>
                  </div>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  const url = getHotmartLink(planData.name);
  ...
  const handlePlanPayment = async (planName: string, finalPrice: number) => {
    try {
      console.log('🧾 Plano e valor para pagamento:', { planName, finalPrice, finalPriceInReais: finalPrice / 100 });
      const url = getHotmartLink(planName);
      if (!url) {
        alert('Link de pagamento não disponível para este plano.');
        return;
      }
      console.log('🔗 Abrindo Hotmart:', url);
      window.open(url, '_blank');
    } catch (error: any) {
      console.error('❌ Erro ao abrir link de pagamento:', error);
      alert('Não foi possível abrir o link de pagamento. Tente novamente.');
    }
  };
  ...
  const handleGenerateBets = useCallback(async () => {
    ...
    const allowTrial = isTrialMode && isBarcaOrRealFixture(selectedFixture);
    
    // Verificar se usuário está logado, exceto no teste grátis permitido
    if (!currentUser && !allowTrial) {
      setError('Usuário não autenticado. No teste grátis, gere apostas apenas para jogos do Barcelona ou do Real Madrid.');
      return;
    }
    ...
    // Verificar créditos antes de gerar apostas (exceto no teste grátis permitido)
    if (!allowTrial) {
      try {
        console.log('🔍 Verificando créditos para usuário:', currentUser!.id);
        const { hasCredits, creditsLeft: left, plan: userPlan } = await checkUserCredits(currentUser!.id);
        creditsLeft = left;
        plan = userPlan;
        console.log('🔍 Resultado da verificação:', { hasCredits, creditsLeft: left, plan: userPlan });
        
        if (!hasCredits && userPlan !== 'Premium') {
          console.log('❌ Usuário sem créditos disponíveis');
          setError(`❌ Sem créditos disponíveis! Você tem ${left} créditos restantes. Troque o plano ou espere amanhã.`);
          return;
        }
        
        console.log('✅ Usuário pode gerar apostas. Créditos:', left, 'Plano:', userPlan);
      } catch (error) {
      ...
      console.log('🧪 Teste grátis: gerando apostas sem verificar créditos');
    }
    ...
    try {
      ...
      if (availableBets.length > 0) {
        ...
        // Incrementar análises apenas fora do teste grátis
        if (!allowTrial && currentUser) {
        ...
        // Descontar crédito após gerar apostas com sucesso (apenas para não-Premium e fora do teste)
        if (!allowTrial && plan !== 'Premium') {
          ...
          console.log('🧪 Teste grátis - sem desconto de créditos');
        }
  ...
  }, [selectedFixture, gameAnalysis, generateBetSuggestions, getTeamLastMatches, analyzeRecentForm, currentUser, isTrialMode]);
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-white">
                    ...
                    <span>
                      <span className="text-white/60 line-through mr-2">R$ {(selectedPlan.name === 'Plano Básico' ? 35 : selectedPlan.name === 'Plano Pro' ? 45 : 75).toFixed(2)}</span>
                      R$ 10,00
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between text-green-400">
                    ...
                    <span>-R$ {((selectedPlan.name === 'Plano Básico' ? 35 : selectedPlan.name === 'Plano Pro' ? 45 : 75) - 10).toFixed(2)}</span>
                  </div>
              ...
              <button
                onClick={() => {
                  const url = getHotmartLink(selectedPlan.name);
                  window.open(url, '_blank');
                }}
  ...
  const handleFixtureAnalysis = useCallback(async (fixture: Fixture) => {
    const allowTrial = isTrialMode && isBarcaOrRealFixture(fixture);

    // Verificar se usuário está logado, exceto no teste grátis permitido
    if (!currentUser && !allowTrial) {
      setError('Usuário não autenticado. No teste grátis, analise apenas jogos do Barcelona ou do Real Madrid.');
      return;
    }

    // Verificar créditos antes de permitir análise (exceto no teste grátis permitido)
    if (!allowTrial) {
      try {
      ...
      // Em teste grátis, bloquear análise se não for jogo do Barcelona ou do Real Madrid
      if (!isBarcaOrRealFixture(fixture)) {
        setError('🔒 Teste grátis: disponível apenas para jogos do Barcelona ou do Real Madrid');
        return;
      }
    }
  ...
  }, [analyzeGame, currentUser, isTrialMode]);
  ...
  // Verificar se o jogo envolve Barcelona ou Real Madrid
  const isBarcaOrRealFixture = (fixture: Fixture) => {
    const home = fixture?.teams?.home?.name?.toLowerCase() || '';
    const away = fixture?.teams?.away?.name?.toLowerCase() || '';
    const hasBarca = home.includes('barcelona') || away.includes('barcelona');
    const hasReal = home.includes('real madrid') || away.includes('real madrid');
    return hasBarca || hasReal;
  };
  ...
  const handleTeamSelect = useCallback((team: Team) => {
    if (isTrialMode) {
      ...
      const isAllowedTeam = nameLower.includes('barcelona') || nameLower.includes('real madrid');
      if (!isAllowedTeam) {
        setError('🔒 Teste grátis: escolha Barcelona ou Real Madrid para continuar');
        return;
      }
    }
    searchTeamFixtures(team);
  }, [searchTeamFixtures, isTrialMode]);
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-400">
                    ...
                    <div className={`text-xs font-medium ${
                      ...
                      {(isTrialMode && !isBarcaOrRealFixture(fixture))
                        ? '🔒 Teste grátis: apenas jogos do Barcelona ou do Real Madrid'
                        : (currentUser && currentUser.plan !== 'Premium' && currentUser.credits <= 0)
                          ? '❌ Sem créditos'
                          : '🔍 Analisar Jogo'
                      }
          ...
          <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center min-h-[60vh] sm:min-h-[70vh] lg:min-h-[80vh]">
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button 
                  onClick={() => { setIsTrialMode(true); setShowSearchInterface(true); setError(''); }}
                  className="bg-white/10 hover:bg-white/20 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl lg:rounded-2xl font-bold text-base sm:text-lg transition-all duration-300 border border-white/20 hover:shadow-lg transform hover:scale-105"
                >
                  Teste Grátis
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 max-w-6xl mx-auto mb-8 lg:mb-12">
              ...
              <div className="group bg-black/40 backdrop-blur-xl rounded-2xl p-6 sm:p-8 border border-white/10 hover:border-[#FF3002]/30 transition-all duration-500 hover:shadow-2xl hover:shadow-[#FF3002]/20 transform hover:-translate-y-2 relative">
                <div className="text-center mb-8">
                  ...
                  <div className="mb-6">
                    <span className="text-gray-400 text-lg line-through mr-3">R$ 35</span>
                    <span className="text-4xl sm:text-5xl font-bold text-[#FF3002]">R$ 10</span>
                ...
                <p 
                   className="text-center text-gray-400 text-xs sm:text-sm mt-2 cursor-pointer hover:text-[#FF3002] hover:underline"
                   onClick={() => { setIsTrialMode(true); setShowSearchInterface(true); setError(''); }}
                 >
                   ou teste grátis
                 </p>
              ...
              <div className="group bg-gradient-to-br from-[#FF3002]/20 via-black/60 to-[#FF3002]/10 backdrop-blur-xl rounded-2xl p-6 sm:p-8 border-2 border-[#FF3002]/50 hover:border-[#FF3002] transition-all duration-500 hover:shadow-2xl hover:shadow-[#FF3002]/30 transform hover:-translate-y-2 relative scale-105">
                ...
                <div className="text-center mb-8">
                  ...
                  <div className="mb-6">
                    <span className="text-gray-400 text-lg line-through mr-3">R$ 45</span>
                    <span className="text-4xl sm:text-5xl font-bold text-[#FF3002]">R$ 10</span>
                    <span className="text-gray-400 text-lg">/mês</span>
                  </div>
              ...
              <div className="group bg-black/40 backdrop-blur-xl rounded-2xl p-6 sm:p-8 border border-white/10 hover:border-[#FF3002]/30 transition-all duration-500 hover:shadow-2xl hover:shadow-[#FF3002]/20 transform hover:-translate-y-2 relative">
                <div className="text-center mb-8">
                  ...
                  <div className="mb-6">
                    <span className="text-gray-400 text-lg line-through mr-3">R$ 75</span>
                    <span className="text-4xl sm:text-5xl font-bold text-[#FF3002]">R$ 10</span>
                    <span className="text-gray-400 text-lg">/mês</span>
                  </div>
      ...
      <section className="py-8 sm:py-12 flex-1 relative overflow-visible">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-50">
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            {isTrialMode && !currentUser && (
              <div className="bg-black/40 backdrop-blur-xl border border-yellow-500/30 rounded-2xl p-4 max-w-md sm:max-w-lg mx-auto mb-6">
                <div className="flex items-start space-x-3">
                  <AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6 text-yellow-400 flex-shrink-0 mt-0.5" />
                  <div className="text-left">
                    <p className="text-yellow-300 font-semibold text-sm sm:text-base">Teste grátis ativo</p>
                    <p className="text-gray-300 text-xs sm:text-sm">Você pode analisar apenas jogos do Barcelona ou do Real Madrid. Faça login para liberar todos os jogos.</p>
                  </div>
                </div>
              </div>
            )}
